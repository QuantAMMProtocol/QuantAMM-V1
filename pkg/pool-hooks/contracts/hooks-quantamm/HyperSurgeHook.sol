...
library HyperPrice {
    ...
    function spot(uint32 pairIndex) internal view returns (uint64 price) {
        (bool ok, bytes memory out) =
            PRECOMPILE.staticcall(abi.encode(pairIndex));
 ...
 */
contract HyperSurgeHook is
    BaseHooks,
    VaultGuard,
    SingletonAuthentication, Ownable
{
    ...
    {
        // Initialise with defaults; governance / feeManager may tune later.
        _poolConfig[pool] = PoolConfig({
            pairIndex: 0, // ← MUST be set later with `setPairIndex`
    ...
    function setMaxSurgeFeePercentage(address pool, uint256 pct)
        external onlyOwner
    {
        _ensurePct(pct);
    ...
    function setSurgeThresholdPercentage(address pool, uint256 pct)
        external onlyOwner
    {
        _ensurePct(pct);
        ...
        returns (bool, uint256)
    {
        ...
        // 2. Derive pool-implied price and peg price.
        uint256 poolPx = _poolImpliedPrice(balances);  // 18-dec USD
        uint256 pegPx  = _pegPrice18(cfg.pairIndex);   // 18-dec USD

        // 3. Relative deviation (|pool-peg| / peg).
        uint256 deviation = poolPx > pegPx
            ? (poolPx - pegPx).divDown(pegPx)
            : (pegPx - poolPx).divDown(pegPx);
        ...
        // surgeFee = static + (max-static) * (dev-thresh)/(1-thresh)
    ...
    ///      Requires both balances already scaled to 18 decimals.
    function _poolImpliedPrice(uint256[2] memory bal)
