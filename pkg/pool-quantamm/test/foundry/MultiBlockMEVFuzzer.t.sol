...
contract MultiBlockMEVFuzzer is QuantAMMWeightedPoolContractsDeployer, BaseVaultTest, QuantAMMMathGuard {
    ...
    function _getInitialBalances(uint256 numTokens, BalanceFuzzParams memory balanceParams) internal returns(uint256[] memory){
    ...
    function _createPoolParams(PoolFuzzParams memory params) 
        internal returns (QuantAMMWeightedPoolFactory.NewPoolParams memory) {
        
        // Create base params first
        QuantAMMWeightedPoolFactory.NewPoolParams memory baseParams = _createBaseParams(params.numTokens, params.maxSwapfee);
    ...
    function _createBaseParams(uint256 numTokens, uint64 maxSwapFee) internal returns (QuantAMMWeightedPoolFactory.NewPoolParams memory) {
    ...
    function _createInitialWeights(uint256 numTokens) internal pure returns (uint256[] memory initialWeightsUint, int256[] memory initialWeights) {
    ...
    function _createPoolSettings(PoolFuzzParams memory params) 
        ...
        return IQuantAMMWeightedPool.PoolSettings({
    ...
    uint64 absWeightGuardRail
        ...
        for(uint i = 0; i < numTokens - 1; i++) {
            ...
            int256 weight = i == 0 ? weights.weight0 :
                        i == 1 ? weights.weight1 :
                        i == 2 ? weights.weight2 :
                        i == 3 ? weights.weight3 :
                        i == 4 ? weights.weight4 :
                        i == 5 ? weights.weight5 :
                        i == 6 ? weights.weight6 : weights.weight7;
    ...
    function _addLiquidityUnbalancedSingleToken(address poolAddress, uint256 tokenIndex, uint256 amount) internal returns (uint256) {
        uint256[] memory amounts = new uint256[](IQuantAMMWeightedPool(poolAddress).getQuantAMMWeightedPoolImmutableData().tokens.length);
        amounts[tokenIndex] = amount;
        return router.addLiquidityUnbalanced(poolAddress, amounts, 0, false, "");
    }

    function _removeLiquidityUnbalancedSingleToken(address poolAddress, uint256 tokenIndex, uint256 bptRedeemed) internal returns (uint256) {
        IERC20[] memory poolTokens = IQuantAMMWeightedPool(poolAddress).getQuantAMMWeightedPoolImmutableData().tokens;
        ...
        uint256 amountsOut = router.removeLiquiditySingleTokenExactIn(poolAddress,  bptRedeemed, poolTokens[tokenIndex],  minAmountsOut[tokenIndex] /* min amount out*/, false, "");
        return amountsOut;
    }

    function _checkAllowances(address user, address ammPool, uint256 amount) internal view {
    ...
    function _validateAndBoundParams(FuzzParamsSingleSameToken memory params) private view {
    ...
    function _setupPool(FuzzParamsSingleSameToken memory params, uint256[] memory initialBalances) private returns (address, uint256) {
    ...
    uint256 bptAmountReceivedOnDeposit) private view {
    ...
    address ammPool,
    uint40 timestamp) private returns (int256[] memory poolStartWeights, int256[] memory firstRandomWeights) {
    ...
    FuzzParamsSingleSameToken memory params,
    address ammPool,
    uint40 timestamp) private returns (uint256 bptReceived, uint256 boundedDeposit) {
        ...
        vm.startPrank(alice);
        bptReceived = _addLiquidityUnbalancedSingleToken(
            ammPool,
            params.tokenIndex,
            boundedDeposit
        );
    ...
    function _verifyAliceDeposit(
    address ammPool, 
    uint256 tokenIndex,
    uint256 bptReceived) private view {
    ...
    function _handleSecondWeightChange(FuzzParamsSingleSameToken memory params, address ammPool, uint40 timestamp, int256[] memory firstRandomWeights) private 
                                       returns (int256[] memory secondRandomWeights) {
    ...
    function _verifyInterpolationTimes(address ammPool, uint40 timestamp) private view {
        IQuantAMMWeightedPool.QuantAMMWeightedPoolDynamicData memory poolData = 
            IQuantAMMWeightedPool(ammPool).getQuantAMMWeightedPoolDynamicData();

        assertEq(
            poolData.lastUpdateIntervalTime, 
            timestamp, 
            "Last update interval time mismatch"
        );
    ...
    address ammPool,
    uint40 timestamp,
    uint256 bptReceived) private returns(uint256) {

        console.log("---After Approvals---");
        _checkAllowances(alice, ammPool, bptAmount);
        ...
        uint256 maxSafeWithdrawal = currentBalances[params.tokenIndex].mulDown(FixedPoint.ONE - (minInvariantRatio + 100)); // @note 100 added to prevent rounding errors        
        ...
        vm.startPrank(alice);
        uint256 amountOut = _removeLiquidityUnbalancedSingleToken(
            ammPool,
            params.tokenIndex,
            boundedBptAmount
        );
    ...
    address ammPool,
    FuzzParamsSingleSameToken memory params,
    uint256 amountOut) private view { 

        IERC20 token = IQuantAMMWeightedPool(ammPool).getQuantAMMWeightedPoolImmutableData().tokens[params.tokenIndex];
        assertEq(token.balanceOf(alice), amountOut, "Alice's token balance should be equal to amount out after withdrawal");
    }
    ...
    function _logFuzzParams(FuzzParamsSingleSameToken memory params, int256 profit, uint256 minThresholdProfit) private {
        console.log("**********Fuzz Params************* ");
        console.logString(string.concat("Num Tokens: ", vm.toString(params.poolParams.numTokens)));
        console.logString(string.concat("Absolute Weight Guard Rail:", vm.toString(params.poolParams.absoluteWeightGuardRail/1e16), "%"));
    ...
    function testSingleTokenSameAssetMEV_Fuzz(FuzzParamsSingleSameToken memory params) public{
        ...
        //11. Set weights to the second set of random weights
         ...
         poolState.secondRandomWeights = _handleSecondWeightChange(params, poolState.quantAMMWeightedPool, poolState.timestamp, poolState.firstRandomWeights);
        ...
        //14. Alice withdraws
        uint256 aliceBalanceBefore = IQuantAMMWeightedPool(poolState.quantAMMWeightedPool).getQuantAMMWeightedPoolImmutableData().tokens[params.tokenIndex].balanceOf(alice);
        uint256 amountOut = _handleAliceWithdrawal(params, poolState.quantAMMWeightedPool, poolState.timestamp, bptReceived);
