...
contract ChannelFollowingUpdateRuleTest is Test, QuantAMMTestUtils {
    ...
    function testFuzz_channelFollowing_reasonableRanges(FuzzRuleParams memory params) public {
        ...
        for(uint i = 0; i < boundNumParameters; i++){
            parameters[0][i] = PRBMathSD59x18.fromInt(bound(params.kappa, 1, 5));
        }

        //Width
        parameters[1] = new int256[](boundNumParameters);
        for(uint i = 0; i < boundNumParameters; i++){
            parameters[1][i] = PRBMathSD59x18.fromInt(bound(params.width, 1, 100));
            vm.assume(parameters[1][i] != 0);
        }

        //Amplitude
        parameters[2] = new int256[](boundNumParameters);
        for(uint i = 0; i < boundNumParameters; i++){
            parameters[2][i] = PRBMathSD59x18.fromInt(bound(params.amplitude, 1, 10));
        }
        ...
        // Inverse Scaling
        parameters[4] = new int256[](boundNumParameters);
        for(uint i = 0; i < boundNumParameters; i++){
            parameters[4][i] = 0.5415e18;
        }
        ...
        parameters[6][0] = PRBMathSD59x18.fromInt(useRawPriceInt);
