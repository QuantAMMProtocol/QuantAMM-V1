...
contract E2eSwapQuantAMMTest is E2eSwapTest, QuantAMMWeightedPoolContractsDeployer {
    ...
    function _createPool(address[] memory tokens, string memory label) internal override returns (address newPool, bytes memory poolArgs)  {
        ...
        return createMockQuantAMMPool(tokens, label, rateProviders, vault, lp);
    }
    ...
    function createPoolFactory() internal override returns (address) {
        return address(deployQuantAMMWeightedPoolFactory(IVault(address(vault)), 365 days, "Factory v1", POOL_VERSION));
    }
    ...
    function _setPoolBalancesWithDifferentWeights(
        ...
        MockQuantAMMWeightedPool(pool).setNormalizedWeights(newWeights);
