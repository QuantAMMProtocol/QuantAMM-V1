...
contract FungibilityQuantAMMInterpolationTest is FungibilityTest, QuantAMMWeightedPoolContractsDeployer {
    /// @notice Overrides BaseVaultTest _createPool(). This pool is used by FungibilityTest.
    function _createPool(address[] memory tokens, string memory label) internal override returns (address) {
        IRateProvider[] memory rateProviders;
        address pool =  address(createQuantAMMPoolInterpolating(tokens, label, rateProviders, vault, lp));
